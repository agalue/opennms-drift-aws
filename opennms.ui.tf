...
data "template_file" "opennms_ui" {
  ...
  vars {
    ...
    postgres_grafana_url = "jdbc:postgresql://grafana:grafana@${join(",", formatlist("%v:5432", keys(var.pg_ip_addresses)))}/grafana?targetServerType=master&loadBalanceHosts=false"
